# Deploying the Smart Agent in Amazon EKS

* Use the SignalFx Helm chart to install the Smart Agent in EKS
...
The Realm can be found in the middle of the page within the Organizations section.  In this example it is `us2`.
...
Create the following variables to use in the proceeding helm install command, replacing the highlighted `{==VARIABLE==}` with the appropriate values. For instance, if your realm is `us2`, you would run `export REALM=us2` and for `eu0` run `export REALM=eu0`.
...
=== "Output"
    ...
    signalfx-agent time="2020-05-27T20:52:12Z" level=info msg="Setting cluster:sedj-k8s-cluster property on host:sedj dimension"                                                                    │
...
Validate that your cluster is discovered and reporting by finding your cluster
